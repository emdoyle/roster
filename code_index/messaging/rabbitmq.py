...
from code_index import constants, errors, settings
from code_index.util.async_helpers import make_async
...
async def setup_rabbitmq(retries: int = 10, delay: float = 2):
    for i in range(retries):
        try:
            ...
            logger.debug(f"(setup_rabbitmq): Could not connect to RabbitMQ")
            if i < retries - 1:
...
async def teardown_rabbitmq():
    await get_rabbitmq().disconnect()
